...
function tbEtiketAc(r){
  ...
  acModal(`<h2 class="serif" style="margin:0 0 4px">${r+1}. El — Not</h2>
    ...
    <div class="row wrap" id="tbEtKim" style="gap:6px;margin:6px 0 12px">
      ${[0,1].map(i=>(c.takimlar[i].oyuncular||[]).map(oid=>`<span class="chip ${el.etiket&&el.etiket.oy===oid?'on':''}" data-oy="${oid}" data-i="${i}"
        onclick="chipTek(this)"><span class="pill ${i?'blue':'green'}">${esc(c.takimlar[i].ad)}</span> ${esc(ad(oid))}</span>`).join('')).join('')}
    </div>
