...
const TAKIMLAR={
  FB:  {ad:'Fenerbahçe',  kisa:'FB',  r1:'#164F8C', r2:'#F4D03F', yazi:'#fff',
        laf:['Masa Kadıköy’e taşındı.','Şampiyonluk masada gelir.','Sarı-lacivert sicil tutuyor.']},
  GS:  {ad:'Galatasaray', kisa:'GS',  r1:'#A4123F', r2:'#F5B301', yazi:'#fff',
        laf:['Sürü içinde tek Aslan.','Tek başına dört yıldız taşıyor.','Aslan masaya kondu.']},
  BJK: {ad:'Beşiktaş',    kisa:'BJK', r1:'#111111', r2:'#e8e8e8', yazi:'#fff',
...
function takimRozet(id,boy){
  ...
  const f=boy||10.5;
  return `<span title="${esc(t.ad)}" style="display:inline-flex;align-items:center;gap:0;
    font-size:${f}px;font-weight:800;letter-spacing:.3px;border-radius:5px;overflow:hidden;
    border:1px solid rgba(255,255,255,.12);vertical-align:middle">
    <span style="background:${t.r1};color:${t.yazi};padding:1px 4px">${t.kisa}</span>
    <span style="background:${t.r2};width:5px;align-self:stretch"></span></span>`;
}
...
function takimRozetKod(kod,boy){
  ...
  const f=boy||10.5;
  return `<span title="${esc(t.ad)}" style="display:inline-flex;align-items:center;
    font-size:${f}px;font-weight:800;letter-spacing:.3px;border-radius:5px;overflow:hidden;
    border:1px solid rgba(255,255,255,.12);vertical-align:middle">
    <span style="background:${t.r1};color:${t.yazi};padding:1px 4px">${t.kisa}</span>
    <span style="background:${t.r2};width:5px;align-self:stretch"></span></span>`;
