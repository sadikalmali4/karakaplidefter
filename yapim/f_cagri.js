...
const cagriBenim = a=>{
  const t=(a.tepkiler||[]).find(x=>x.profilId===OTURUM.id);
...
function cagriAc(){
  ...
  acModal(`<div class="center"><div style="font-size:30px">📣</div>
    ...
    ${yerSecici(sonYer())}
    <div class="field"><label class="fl">Eşler</label>
      <div class="seg" id="cgEs">
        <button class="on" data-e="kura" onclick="segSec(this)">🎲 Kurayla</button>
        <button data-e="belli" onclick="segSec(this)">🤝 Belli</button>
      </div>
      <div class="xs dim" style="margin-top:6px">Kurayla derseniz katılanlar arasından çekilir.
        Belli derseniz masada konuşulur, uygulama karışmaz.</div></div>
...
async function cagriKaydet(){
  ...
  const metin=`${saatMetni(zaman.toISOString())} · ${yer} — masa kuruluyor.`+
    (es==='kura'?' Eşler kurayla çekilecek.':' Eşler masada belirlenecek.')+
    (not?` ${not}`:'');
...
/* --------------------------- cevap --------------------------- */
async function cagriCevapla(akisId,isaret,sessiz){
  try{
    /* Tek cevap: önce varsa kaldır, sonra yaz */
    await sb.from('akis_tepkileri').delete()
      .eq('akis_id',akisId).eq('profil_id',OTURUM.id);
    if(isaret){
      const {error}=await sb.from('akis_tepkileri')
        .insert({akis_id:akisId,profil_id:OTURUM.id,emoji:isaret});
      if(error) throw error;
    }
    if(!sessiz){ await yenile(true);
      toast(isaret===CAGRI_VAR?'Katılıyorsun, yazıldı.':'Katılmıyorsun, yazıldı.'); }
    return true;
...
function cagriKart(){
  ...
  return `<div class="card" style="border-color:${gecti?'var(--line)':'var(--gold)'}">
    <div class="row" style="justify-content:space-between;align-items:flex-start">
      ...
      <span class="pill ${c.es==='kura'?'gold':''}">${c.es==='kura'?'🎲 Kurayla':'🤝 Eşler belli'}</span>
    </div>
