...
function kartSvg(c){
  ...
  const orta=G/2;
  /* Mühür rengi seçili temadan gelsin — KIRMIZI DEĞİL (kullanıcı bordoyu
     kaldırttı). Varsayılan yeşil çuha; okunamayacak kadar açık gelirse altına düş. */
  let vurgu='#2E6B4F';
  try{ vurgu=(getComputedStyle(document.documentElement).getPropertyValue('--accent')||'').trim()||vurgu; }catch(e){}
  ...
  <!-- başlık -->
  <circle cx="${orta}" cy="150" r="34" fill="none" stroke="${vurgu}" stroke-width="4"/>
  <text x="${orta}" y="150" fill="${vurgu}" font-family="Georgia,serif" font-weight="700" font-size="34" text-anchor="middle" dominant-baseline="central">P</text>
